Properties {
	...
	$solution_name = "$base_dir\WebApiContrib.Formatting.MsgPack.sln"
	$test_dll = "$build_artifacts_dir\WebApiContrib.Formatting.MsgPack.Tests.dll"
...
Task NuGetBuild -depends Clean {
	& $nuget_exe pack "$base_dir/src/WebApiContrib.Formatting.MsgPack/WebApiContrib.Formatting.MsgPack.csproj" -Build -OutputDirectory $build_artifacts_dir -Verbose -Properties Configuration=Release
}
